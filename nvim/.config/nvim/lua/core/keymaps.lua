vim.keymap.set("n", "<leader>e", ":lua Snacks.explorer()<CR>", { desc = "Toggle file tree" })
...
function _G.set_markdown_keymaps()
	...
	if ok and autolist and autolist.operations then
		...
		vim.keymap.set("n", "dd", "dd<cmd>AutolistRecalculate<cr>", { buffer = true, desc = "Delete and recalculate" })
		vim.keymap.set("v", "d", "d<cmd>AutolistRecalculate<cr>", { buffer = true, desc = "Delete and recalculate" })
	else
...
if not buffer_utils_loaded then
	...
	if _G.GotoBuffer then
		...
		local function safe_buffer_prev()
			...
			for _, buf in ipairs(buffers) do
				-- Only include normal buffers (not terminals)
				-- if vim.api.nvim_buf_is_valid(buf.bufnr) and vim.api.nvim_buf_get_option(buf.bufnr, "buftype") == "" then
				if
					vim.api.nvim_buf_is_valid(buf.bufnr)
					and vim.api.nvim_get_option_value("buftype", { buf = buf.bufnr }) == ""
				then
