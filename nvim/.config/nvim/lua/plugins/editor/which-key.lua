	...
	config = function(_, opts)
		...
		wk.add({
			{ "<leader>g", group = "git", icon = "󰊢", mode = { "n", "v" } },
			{
				"<leader>gb",
				function()
					Snacks.picker.git_branches({ remote = true })
				end,
				desc = "branches",
				icon = "󰘬",
			},
		...
		wk.add({
			...
			{ "<leader>is", "<cmd>LspRestart<CR>", desc = "restart lsp", icon = "󰜉" },
			{
				...
				function()
					...
					if #clients > 0 then
						vim.cmd("LspStop")
						require("util.notifications").lsp(
							"LSP stopped",
							require("util.notifications").categories.USER_ACTION
						)
					else
						vim.cmd("LspStart")
						require("util.notifications").lsp(
		...
		wk.add({
			...
			{
				"<leader>pv",
				"<cmd>TermExec cmd='sioyek %:p:r.pdf &' open=0<CR>",
				desc = "view",
				icon = "󰛓",
				cond = is_pandoc_compatible,
			},
		...
		wk.add({
			...
			{
				"<leader>rl",
				"<cmd>lua require('util.diagnostics').show_all_errors()<CR>",
				desc = "show linter errors",
				icon = "󰅚",
			},
			...
			{
				"<leader>rK",
				"<cmd>TermExec cmd='rm -rf ~/.local/share/nvim/lazy && rm -f ~/.config/nvim/lazy-lock.json' open=0<CR>",
				desc = "wipe plugins and lock file",
				icon = "󰩺",
			},
